...
def main(args):
    ...
    with torch.no_grad():
        for pos, batch in enumerate(val_loader):
            ...
            x, _, path = batch
            print(x.shape)
            feature_map, feature = model.backbone(x)
            print(feature_map.shape, feature.shape)
            #torch.Size([128, 2048, 16, 8]) torch.Size([128, 2048]) 
            # map: [batch dim:128, feature dim:2048, feature_map_H:16, feature_map_W:8]
            #print(y[1][0].shape)
